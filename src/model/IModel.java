...
/**
 * Represents the ImageEditor of this image processor. The interface contains methods that should
 * be used when trying to load, retrieve, edit, and save an image. Edits that can be done to an
 * image include visualizing its color components, luma, intensity, value, brightening and
 * darkening, flipping, and applying filters such as greyscale and sepia.
 */
public interface IModel {
  ...
  /**
   * Brightens the image by the given value by adding the value to each color value for
   * every pixel.
   *
  ...
  /**
   * Darkens the image by the given value by subtracting the value to each color value for
   * every pixel.
   *
  ...
  /**
   * Changes the value of all the colors of a pixel to the color value that is the highest value
   ...
   * pixels in an image. The formula of the luma of an image is (.2126 * redColorValue) +
   * (.7152 * blueColorValue) + (.0722 * greenColorValue).
   *
   * @param newName the new name of the image for temporary storage purposes
   */
  ...
  /**
   * Saves the image, chosen by the user, to a specific path in their files, that the user will
   * also choose.
