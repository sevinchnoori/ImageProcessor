...
//    this.setResizable(true); // does allow user to resize window - CHECK HOW THIS WORKS ONCE IMAGE IS ON WINDOW
    // if this turns true - add this.setMinimumSize(new Dimension(,)
    ...
    hRed = (new Histogram(this.model.retrieveImage()).makeHisto(IHistogram.HistoType.RED_HISTOGRAM));
    jRed = new ImageIcon(hRed);
    lRed = new JLabel(jRed);
    histoRow.add(lRed);

    hGreen = (new Histogram(this.model.retrieveImage()).makeHisto(IHistogram.HistoType.GREEN_HISTOGRAM));
    jGreen = new ImageIcon(hGreen);
    lGreen = new JLabel(jGreen);
    histoRow.add(lGreen);

    hBlue = (new Histogram(this.model.retrieveImage()).makeHisto(IHistogram.HistoType.BLUE_HISTOGRAM));
    jBlue = new ImageIcon(hBlue);
    lBlue = new JLabel(jBlue);
    histoRow.add(lBlue);

    hIntensity = (new Histogram(this.model.retrieveImage()).makeHisto(IHistogram.HistoType.INTENSITY_HISTOGRAM));
    ...
    this.pack(); // resizes the window so it only uses the space needed for its components - don't need to think about calculations of where everything needs to go
