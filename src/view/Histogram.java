      ...
      for (int x = 0; x < this.img.length; x = x + 1) {
        for (int y = 0; y < this.img[0].length; y = y + 1) {
          ...
          if (type == (HistoType.RED_HISTOGRAM)) {
            redStore.put(img[x][y].getR(), redStore.getOrDefault(img[x][y].getR(), 0) + 1);
            // redStore.get(255) to get the amount when making the bargraph
          } else if (type == (HistoType.GREEN_HISTOGRAM)) {
            greenStore.put(img[x][y].getG(), greenStore.getOrDefault(img[x][y].getG(), 0) + 1);
          } else if (type == (HistoType.BLUE_HISTOGRAM)) {
            blueStore.put(img[x][y].getB(), blueStore.getOrDefault(img[x][y].getB(), 0) + 1);
          } else if (type.equals(HistoType.INTENSITY_HISTOGRAM)) {
            intensityStore.put(img[x][y].getIntensity(), intensityStore.getOrDefault(img[x][y].getIntensity(), 0) + 1);
          }
        } // inner loop
      } // outer loop
